...
class ListaEnlazada {
	...
	public:
		...
		void invert(int);
		void sort(int);
		void mezcla(ListaEnlazada, int);
		void convertir(int);
		void concat(ListaEnlazada, int);
...
template<typename T>
void ListaEnlazada<T>::concat(ListaEnlazada list, int tipo)
{
...
template<typename T>
void ListaEnlazada<T>::convertir(int tipo)
{
	Nodo<T> *p = inicio;
	if(tipo == 1) {
		Pilas_dinamicas <T> PilaL;
		while(p) {
			PilaL.Anadir(p->getValor());
			p = p->getSiguiente();
		}
//		PilaL.Imprimir();
	} else if(tipo == 2) {
		Colas_dinamicas <T> ColaL;
		while(p) {
			ColaL.Anadir(p->getValor());
			p = p->getSiguiente();
		}
//		ColaL.Imprimir();
	} else if(tipo == 3){
		ListaEnlazada <T> ListaL;
		while(p) {
			ListaL.Anadir(p->getValor());
			p = p->getSiguiente();
		}
//		ListaL.Imprimir();
	}
...
template<typename T>
void ListaEnlazada<T>::mezcla(ListaEnlazada list_2, int tipo)
{
    Nodo<T> *temp = inicio;
...
template<typename T>
void ListaEnlazada<T>::sort(int tipo)
{
    T temp_data;
...
template<typename T>
void ListaEnlazada<T>::invert(int tipo)
{
    Nodo<T> *prev = NULL;
    Nodo<T> *next = NULL;
    Nodo<T> *temp = inicio;
