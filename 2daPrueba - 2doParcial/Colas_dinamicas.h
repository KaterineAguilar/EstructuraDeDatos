...
class Colas_dinamicas {
	...
	public:
		...
		void invert(int);
		void sort(int);
		void mezcla(Colas_dinamicas, int);
		void convertir(int);
		void concat(Colas_dinamicas, int);
...
template<typename T>
void Colas_dinamicas<T>::concat(Colas_dinamicas col, int tipo)
{
...
template<typename T>
void Colas_dinamicas<T>::convertir(int tipo)
{
	nodo<T> *p = primero;
	if(tipo == 1) {
		Pilas_dinamicas <T> PilaC;
		while(p) {
			PilaC.Anadir(p->getValor());
			p = p->getSiguiente();
		}
//		PilaC.Imprimir();
	} else if(tipo == 2) {
		Colas_dinamicas <T> ColaC;
		while(p) {
			ColaC.Anadir(p->getValor());
			p = p->getSiguiente();
		}
//		ColaC.Imprimir();
	} else {
		ListaEnlazada <T> ListaC;
		while(p) {
			ListaC.Anadir(p->getValor());
			p = p->getSiguiente();
		}
//		ListaC.Imprimir();
	}
...
template<typename T>
void Colas_dinamicas<T>::mezcla(Colas_dinamicas list_2, int tipo)
{
    nodo<T> *temp = primero;
...
template<typename T>
void Colas_dinamicas<T>::sort(int tipo)
{
    T temp_data;
...
template<typename T>
void Colas_dinamicas<T>::invert(int tipo)
{
    nodo<T> *prev = NULL;
    nodo<T> *next = NULL;
    nodo<T> *temp = primero;
